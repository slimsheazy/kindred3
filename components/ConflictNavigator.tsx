...
Strict adherence to silence during GROUNDING is non-negotiable. Only intervene when the stability threshold is breached.`,
  ...
  const getStabilityColor = () => {
    if (phase === 'intervention') return '#FF85B3'; 
    if (stability > 0.8) return '#A8FFB5'; 
  ...
  if (isPreflight) {
    return (
      <div className="fixed inset-0 z-[100] bg-[#121212] flex flex-col items-center justify-center p-8 animate-fade-in text-center">
        <h2 className="text-clamp-6xl font-light mb-8 text-[#FDFCF0]">Threshold.</h2>
        <div className="max-w-xs space-y-8 w-full">
            <p className="text-2xl text-white/60 italic font-light leading-relaxed">
            ...
            <div className="space-y-6 pt-8">
                <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest heading-font px-4">
                    <span className="text-white/30">Mic Permission</span>
                    <span className={micPermission === 'granted' ? 'text-[#A8FFB5]' : 'text-[#FF85B3]'}>
                        {micPermission === 'granted' ? 'Ready' : 'Pending'}
                    </span>
                </div>
                <div className="w-full h-[1px] bg-white/10" />
                <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest heading-font px-4">
                    <span className="text-white/30">Oracle Sync</span>
                    <span className="text-[#A8FFB5]">Calibrated</span>
                </div>
            </div>
            ...
            <button 
                ...
                className="w-full py-6 bg-white text-black rounded-full font-bold uppercase text-xs tracking-[0.3em] shadow-2xl disabled:opacity-10 transition-all mt-12 heading-font"
            >
                Initiate Safe Space
            </button>
            <button onClick={onClose} className="text-xs font-bold uppercase tracking-widest text-white/30 hover:text-white transition-colors py-4">Return to Home</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#121212] flex flex-col items-center justify-center p-6 animate-fade-in overflow-hidden">
      <div 
      ...
      <div className="text-center w-full max-w-lg z-10">
        {phase !== 'grounding' && (
          <header className="mb-16 animate-fade-in">
            <h2 className="text-clamp-5xl font-light mb-4 text-[#FDFCF0]">
              {phase === 'mediation' ? 'The Floor.' : 'Pause.'}
            </h2>
            <p className="text-xs font-bold uppercase tracking-[0.4em] text-white/30 heading-font">
        ...
        <div className="relative h-72 flex items-center justify-center mb-12">
            ...
            <div 
                className={`w-40 h-40 rounded-full border border-white/5 flex items-center justify-center transition-all duration-[3000ms] ${phase === 'grounding' ? 'animate-[pulse_8s_infinite] scale-[1.8] border-white/10' : ''}`}
                style={{ borderColor: `${getStabilityColor()}20` }}
            >
               {activeSpeaker && phase !== 'grounding' && (
                 <span className="text-xs font-bold uppercase tracking-[0.2em] text-white/40 heading-font animate-fade-in">
                   {activeSpeaker}
                 </span>
               )}
               {phase === 'grounding' && (
                 <span className="text-xs font-bold uppercase tracking-[0.4em] text-[#A8FFB5]/50 heading-font">Grounding</span>
               )}
            </div>

            {stability < 0.7 && phase !== 'grounding' && (
                <div className="absolute top-0 text-xs font-bold uppercase tracking-widest text-orange-400/60 animate-pulse">
                  Unstable Energy
                </div>
            )}
        </div>
        ...
        {phase !== 'grounding' && (
          <div className="mb-24 px-8 min-h-[5rem] animate-fade-in">
              <p className="text-2xl leading-relaxed text-[#FDFCF0]/80 italic text-center font-light">
        ...
        <div className="flex flex-col items-center gap-6">
            <button 
                ...
                className={`text-xs font-bold uppercase tracking-widest transition-all heading-font ${phase === 'grounding' ? 'text-white/10 hover:text-white/30 mt-20' : 'text-white/30 hover:text-white border-b border-transparent hover:border-white pb-2'}`}
            >
                Dissolve Session
            </button>
        </div>
