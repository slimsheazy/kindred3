...
const Goals: React.FC = () => {
  ...
  const GhostBlueprint = () => (
    <div className="border border-dashed border-current opacity-10 rounded-[2.5rem] p-10 animate-pulse">
        <span className="text-xs font-bold uppercase tracking-[0.4em] mb-4 block">Unmapped Horizon</span>
        <div className="h-12 w-2/3 bg-current opacity-10 rounded-full mb-10" />
        <div className="w-full h-[1.5px] bg-current opacity-20 mb-6" />
        <div className="flex gap-3">
            <div className="w-5 h-5 rounded-sm border border-current opacity-20" />
            <div className="h-4 w-1/3 bg-current opacity-10 rounded-full" />
        </div>
    </div>
  );
    ...
    <div className="px-6 py-12 max-w-xl mx-auto">
      ...
      <form onSubmit={handleAdd} className="mb-24 flex gap-4">
        <input 
            ...
            disabled={isAdding}
            className="flex-grow bg-transparent border-b border-current opacity-20 focus:opacity-100 focus:border-inherit outline-none text-2xl p-4 transition-all placeholder-current placeholder-opacity-25 font-light italic" 
        />
        <button 
            type="submit" 
            disabled={isAdding || !newTitle.trim()}
            className="px-10 border border-current border-opacity-20 text-xs font-bold uppercase tracking-widest rounded-full hover:bg-current hover:text-inherit transition-all opacity-70 h-16 heading-font disabled:opacity-10 shadow-sm"
        >
        ...
        {goals.map(g => (
          <div key={g.id} className="animate-fade-in-up group">
            <div className="flex justify-between items-start mb-8">
              <div>
                  <span className="text-xs font-bold uppercase tracking-widest opacity-30 mb-2 block heading-font">Project Horizon</span>
                  <h3 className="text-clamp-4xl font-light leading-tight group-hover:pl-2 transition-all duration-500">{g.title}</h3>
              </div>
              <span className="text-sm font-bold tracking-[0.2em] opacity-90 heading-font pt-2">{g.progress}%</span>
            </div>

            <div className="w-full h-2 bg-current opacity-5 relative mb-12 overflow-hidden rounded-full">
                <div 
                    className="absolute left-0 top-0 h-full bg-[#3D8C50] dark:bg-[#A8FFB5] transition-all duration-[2000ms] ease-out shadow-[0_0_12px_rgba(168,255,181,0.25)]" 
            ...
            {g.microSteps && g.microSteps.length > 0 && (
                <div className="mb-12 space-y-4 pl-4 border-l border-current border-opacity-10">
                    <span className="text-xs font-bold uppercase tracking-widest opacity-30 mb-4 block heading-font">Technical Specifications</span>
                    {g.microSteps.map(step => (
                        ...
                        >
                            <div className={`w-4 h-4 rounded-sm border border-current border-opacity-20 transition-all ${step.completed ? 'bg-[#3D8C50] dark:bg-[#A8FFB5] border-transparent' : 'group-hover/step:border-opacity-40'}`} />
                            <span className={`text-sm font-mono tracking-tight transition-all ${step.completed ? 'opacity-20 line-through' : 'opacity-80'}`}>
                                {step.text}
                            </span>
                        </button>
                    ))}
            ...
            {g.encouragement && (
                <div className="bg-current bg-opacity-2 p-8 rounded-[2.5rem] border border-current border-opacity-5 animate-fade-in backdrop-blur-sm shadow-inner">
                    <span className="text-xs font-bold uppercase tracking-widest opacity-40 mb-3 block heading-font">Kindred Oracle Insight</span>
                    <p className="text-base italic font-light opacity-70 leading-relaxed">"{g.encouragement}"</p>
                </div>
            )}
            ...
            <div className="mt-10 opacity-0 group-hover:opacity-100 transition-opacity duration-700">
                <input 
                    ...
                    className="w-full h-1 bg-transparent cursor-pointer appearance-none [&::-webkit-slider-runnable-track]:bg-current [&::-webkit-slider-runnable-track]:bg-opacity-10 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[#3D8C50] dark:[&::-webkit-slider-thumb]:bg-[#A8FFB5]" 
                />
            </div>
          </div>
        ))}
